...
const char* _kBeverageNameNames[] = {
  "CAPPUCCINO",
  "LATTE",
  "ESPRESSO",
  "LEMONADE",
...
const std::map<int, const char*> _BeverageName_VALUES_TO_NAMES(::apache::thrift::TEnumIterator(6, _kBeverageNameValues, _kBeverageNameNames), ::apache::thrift::TEnumIterator(-1, nullptr, nullptr));

std::ostream& operator<<(std::ostream& out, const BeverageName::type& val) {
  std::map<int, const char*>::const_iterator it = _BeverageName_VALUES_TO_NAMES.find(val);
  if (it != _BeverageName_VALUES_TO_NAMES.end()) {
    out << it->second;
  } else {
    out << static_cast<int>(val);
  }
...
std::string to_string(const BeverageName::type& val) {
  std::map<int, const char*>::const_iterator it = _BeverageName_VALUES_TO_NAMES.find(val);
  if (it != _BeverageName_VALUES_TO_NAMES.end()) {
    return std::string(it->second);
  } else {
    ...
    switch (fid)
    {
      case 1:
        if (ftype == ::apache::thrift::protocol::T_I32) {
          ...
          this->errorCode = static_cast<ErrorCode::type>(ecast0);
...
uint32_t ServiceException::write(::apache::thrift::protocol::TProtocol* oprot) const {
  ...
  xfer += oprot->writeI32(static_cast<int32_t>(this->errorCode));
...
location::location(const location& other3) noexcept {
  location_id = other3.location_id;
  city = other3.city;
  __isset = other3.__isset;
}
location& location::operator=(const location& other4) noexcept {
